...
# Only these strings are valid regime labels; any other value is treated as missing.
CANONICAL_REGIME_LABELS = frozenset(r.value for r in Regime)
...
class RegimeClassifier:
    ...
    def classify(self, cpi_df: pd.DataFrame) -> pd.DataFrame:
        ...
        # Downstream backtest/metrics code expects clean string labels such as
        # "FIRE", "BOOM", "ICE", "RECOVERY". Pandas / serialization can otherwise
        # leave Enum objects or their repr strings (e.g. "Regime.FIRE"), which
        # then produce spurious categories like "Reg" when grouped or truncated.
        def _to_label(x):
            ...
            if s.startswith("Regime."):
                s = s.split(".", 1)[1]
            if s in CANONICAL_REGIME_LABELS:
                return s
            return None  # drop "Reg", truncated, or any non-canonical label
