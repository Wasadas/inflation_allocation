...
ALLOW_SHORTS = bool(TF_CFG.get("allow_shorts", True))
...
def compute_tsmom_signals(
    ...
    if ALLOW_SHORTS:
        ...
        # Long/flat only: go long when trend is positive, otherwise 0
        raw_signal = (cum_ret > 0).astype(float)
    ...
    # ---- 4. Cap positions: either ±1 (with shorts) or [0, 1] (long/flat) ----
    if ALLOW_SHORTS:
